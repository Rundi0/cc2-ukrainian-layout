...
**Шар A1 (літери).** `q→я w→ш e→е r→р t→т y→и u→у i→і o→о p→п a→а s→с d→д f→ф g→г h→х j→й k→к l→л z→з x→ч c→ц v→в b→б n→н m→м`, плюс `;→ж '→ь ,→є .→ї -→щ /→'` і `\→ю`.

## Єдина зміна в самому CC2

| | профіль A (англ.) | профіль B (укр.) |
|---|---|---|
| права `index out` | `SpaceRight` | `Backslash` → **ю** |

33 літери не вміщаються у 32 символьні слоти шару A1, тож одну клавішу треба
позичити. Беремо правий пробіл — лівий лишається на місці. У Device Manager
шукай дію, підписану просто `\`.

Це має бути `Backslash`, а не `IntlBackslash`: остання — 102-га клавіша
ISO-клавіатур, і Device Manager її не показує, бо на ANSI-розкладці US її
нема де намалювати.

`\` і `|` переїжджають на AltGr тієї ж клавіші — в українському тексті вони не
трапляються. Профіль A не чіпаємо взагалі, тож англійська лишається з двома
пробілами.

**Шар A2 (пунктуація).** Доступ — мізинець назовні (`little out`), будь-яка рука.
Усі клавіші нижче, крім `Backquote`, продубльовані на обидві руки, тож акорд можна
брати різними руками.
