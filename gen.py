...
def expected():
    """scancode / level / codepoint table, for windows\\diagnose.ps1 to verify
    what the installed DLL actually produces.  Level 0=base 1=shift 2=altgr
    3=shift+altgr."""
    out = ['# scancode\tlevel\tcodepoint -- generated by gen.py']
    for key, _, sc, *_ in KEYS:
        for lvl, c in enumerate(levels(key)):
            if c:
                out.append('%02X\t%d\t%04X' % (sc, lvl, ord(c)))
    return '\n'.join(out) + '\n'
...
if __name__ == '__main__':
    ...
    winexp = OUT.parent / 'windows/expected.tsv'
    if winexp.parent.is_dir():
        winexp.write_bytes(expected().encode('ascii'))
        print('%-22s -> diagnose.ps1' % winexp.name)
