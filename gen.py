...
KEYS = [
    ...
    # so on Android this key and Backslash are indistinguishable -> ю wins.
...
def kcm():
    ...
    # Backslash and IntlBackslash are the same keycode on Android; ю wins,
    # so \ and | are simply not typeable in the Ukrainian profile there.
    for key, _xk, _sc, _vk, _mac, kk in KEYS:
        if key not in LAYOUT or key == 'Backslash':
            continue
        b, s, a, sa = levels(key)
        L.append('key %s {' % kk)
        L.append('    label:                              %s'
...
def check():
    ...
    kcm_keys = [k[5] for k in KEYS if k[0] in LAYOUT]
    assert len(kcm_keys) == len(set(kcm_keys)) + 1, 'unexpected kcm collisions'
